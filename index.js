...
io.on('connection', function(client) {
  ...
  // Listens for update channel
  client.on("channelUpdated", function(name, description, channelId) {
    console.log('channel update socket');
    io.emit('userUpdatedChannel')
  })

  // Listens for deleted channel
  client.on("channelDeleted", function(channelId) {
    console.log('channel delete socket');
    io.emit('userDeletedChannel', channelId)
  })

  // Listens for updated message
  client.on("messageUpdated", function(messageInfo) {
    console.log('message updated');
    io.emit('userUpdatedMessage', messageInfo)
  })

  // Listens for deleted message
  client.on("messageDeleted", function(messageId) {
    console.log('message deleted');
    io.emit('userDeletedMessage', messageId)
  })
